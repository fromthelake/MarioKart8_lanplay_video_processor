...
- Initial scan now supports a multi-video shared-process path. It defaults to `2` workers when EasyOCR CUDA is available and `1` worker otherwise; `MK8_PARALLEL_VIDEO_SCAN_WORKERS` can still override it manually. On the current 7-video benchmark set, `2` workers reduced extraction-only runtime from `06:59` to `03:58`, while `3` and `4` workers were slower.
